    ...
    <header className="absolute top-0 left-1/2 -translate-x-1/2 z-[100] w-full max-w-6xl md:mt-4">
      ...
      >
        <button
          ...
          className="absolute right-4 md:right-auto md:left-8 top-1/2 z-40 flex items-center justify-center -translate-y-1/2"
        >
          {isMenuOpen ? (
            ...
            <div className="flex flex-col gap-1.5 items-end">
              <span className="w-8 h-0.5 bg-black rounded-full transition-all"></span>
              <span className="w-8 h-0.5 bg-black rounded-full transition-all"></span>
            </div>
          )}
        </button>

        <Link href="/" aria-label="Go to homepage" className="absolute left-4 md:left-1/2 top-1/2 -translate-y-1/2 md:-translate-x-1/2">
          <svg width="120" height="36" viewBox="0 0 204 61" fill="none" xmlns="http://www.w3.org/2000/svg" className="md:w-[204px] md:h-[61px]">
        ...
        <Link
          href="/contact"
          className="hidden absolute right-4 top-1/2 -translate-y-1/2 bg-black px-6 py-2.5 text-center text-xs font-bold text-white uppercase tracking-wider md:block md:right-8 md:px-8 md:py-3 md:text-sm"
        >
          LET&apos;S WORK TOGETHER
        </Link>
      </div>
